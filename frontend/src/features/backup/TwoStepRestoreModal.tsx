      ...
      <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 bg-slate-50/60">
          <div className="flex items-center space-x-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-50 text-rose-600 border border-rose-100 shadow-2xs">
              <RotateCcw className="h-5 w-5" />
            </div>
            <div>
              <h3 className="text-base font-bold text-slate-900">Phục Hồi Dữ Liệu An Toàn</h3>
              <p className="text-xs text-slate-500">Quy trình xác nhận hai bước (Bước {step}/2)</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="flex items-center gap-2">
            <div
              className={cn(
                "flex-1 flex items-center gap-2 p-2 rounded-xl border text-xs font-semibold transition",
                step === 1
                  ? "bg-amber-50/80 border-amber-200 text-amber-900 ring-1 ring-amber-300"
            ...
            >
              ...
              <span className="truncate">1. Cảnh báo & An toàn</span>
            </div>
            ...
            <div
              className={cn(
                "flex-1 flex items-center gap-2 p-2 rounded-xl border text-xs font-semibold transition",
                step === 2
                  ? "bg-rose-50/80 border-rose-200 text-rose-900 ring-1 ring-rose-300"
            ...
            >
              ...
              <span className="truncate">2. Xác nhận & Giải trình</span>
            </div>
          </div>

          {error && (
            <div className="p-3.5 bg-rose-50 border border-rose-100 rounded-xl text-rose-800 text-xs flex items-start gap-2.5 animate-in fade-in">
          ...
          {step === 1 ? (
            ...
            <form id="restore-form" onSubmit={handleRestoreSubmit} className="space-y-3.5">
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                  Lý do giải trình phục hồi (Tối thiểu 10 ký tự) <span className="text-rose-500">*</span>
                </label>
                <textarea
                  className="w-full rounded-xl border border-slate-200 px-3.5 py-2 text-xs focus:border-rose-500 focus:outline-hidden focus:ring-2 focus:ring-rose-500/10 placeholder:text-slate-400"
                  rows={3}
        ...
        <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/60 px-6 py-3.5">
          ...
          <div className="flex items-center gap-2">
            ...
            {step === 1 ? (
              <button
                type="button"
                className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 transition shadow-xs cursor-pointer"
