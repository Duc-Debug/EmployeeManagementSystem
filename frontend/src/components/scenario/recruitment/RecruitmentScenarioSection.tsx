...
export const RecruitmentScenarioSection: React.FC<RecruitmentScenarioSectionProps> = ({
  ...
  const handleDeleteEmployee = async (emp: SimulatedEmployeeResponse) => {
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn xóa nhân sự giả định "${emp.candidateName}" khỏi kịch bản?`
      )
    ) {
      return;
    }
    ...
    try {
      await deleteSimulatedEmployee(scenarioId, emp.id);
      await loadData();
    } catch (err: unknown) {
      alert(err instanceof Error ? err.message : "Xóa nhân sự giả định thất bại.");
    }
  };

  const formatHours = (val?: number | null) => {
    if (val === undefined || val === null) return "0h";
  ...
  return (
    <div className="mt-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-slate-200">
        ...
        <div className="mt-3 sm:mt-0 flex items-center space-x-2.5">
          <button
            ...
            className="p-2 text-slate-500 hover:text-slate-800 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg shadow-sm transition disabled:opacity-50"
          >
          ...
          {isVT03 && (
            <button
              ...
              className="flex items-center space-x-1.5 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg shadow-sm hover:shadow transition"
            >
      ...
      {error && (
        <div className="flex items-center space-x-2 p-4 bg-rose-50 border border-rose-200 rounded-xl text-rose-700 text-xs">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="mt-2 flex items-baseline space-x-2">
            <span className="text-2xl font-bold font-mono text-slate-900">
              {formatHours(evaluation?.totalOriginalShortfallHours)}
            </span>
          </div>
        ...
        <div className="p-4 bg-white rounded-xl border border-indigo-200 bg-indigo-50/20 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-indigo-700">Giờ bù đắp tuyển mới</span>
            <TrendingUp className="h-4 w-4 text-indigo-600" />
          </div>
          <div className="mt-2 flex items-baseline space-x-2">
            <span className="text-2xl font-bold font-mono text-indigo-700">
              +{formatHours(evaluation?.totalSimulatedCapacityHours)}
            </span>
          </div>
        ...
        <div className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="mt-2 flex items-baseline space-x-2">
            ...
            >
              {formatHours(evaluation?.totalRemainingShortfallHours)}
            </span>
          </div>
          <p className="text-[11px] text-slate-400 mt-1">
            {(evaluation?.totalRemainingShortfallHours ?? 0) > 0
              ? "Cần bổ sung thêm nhân sự"
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
          ...
          <span className="text-[11px] text-slate-400 font-medium">
            Tính toán dựa trên công suất 40h/tuần
          </span>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {loading && !evaluation ? (
                ...
                <tr>
                  <td colSpan={8} className="px-4 py-8 text-center text-slate-400">
                    Không có dữ liệu vai trò nào thiếu hụt trong kịch bản này.
                  </td>
                </tr>
              ) : (
                evaluation.roleEvaluations.map((item: RoleEvaluationItem) => (
                  <tr
                    ...
                    className={`hover:bg-slate-50/70 transition ${
                      item.isRoleBroken ? "bg-rose-50/20" : ""
                    }`}
                  >
                    ...
                    <td className="px-4 py-3 text-right font-mono text-slate-600">
                      {formatHours(item.originalShortfallHours)}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-indigo-600 font-medium">
                      +{formatHours(item.simulatedCapacityHours)}
                    </td>
                    <td className="px-4 py-3 text-right font-mono font-bold">
                      ...
                      >
                        {formatHours(item.remainingShortfallHours)}
                      </span>
                    </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        {item.remainingShortfallHours > 0 && (
                          <button
                            ...
                            className="inline-flex items-center space-x-1 px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-medium rounded-md border border-indigo-200 transition"
                          >
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center space-x-2">
                ...
                simulatedEmployees.map((emp) => (
                  <tr key={emp.id} className="hover:bg-slate-50/70 transition">
                    ...
                    <td className="px-4 py-3 text-right font-mono font-bold text-indigo-700">
                      +{formatHours(emp.totalCapacityHours)}
                    </td>
