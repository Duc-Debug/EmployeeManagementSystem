  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (detailData?.scenario) {
        setNameInput(detailData.scenario.name || "");
        setNoteInput(detailData.scenario.note || "");
      }
    } catch (err: unknown) {
          ...
          {isOwner && (
            <>
              ...
              <button
                ...
                className="rounded-xl bg-slate-900 px-4 py-2 text-xs font-semibold text-white hover:bg-slate-800 transition shadow-xs flex items-center space-x-1.5"
              >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs col-span-2 md:col-span-1">
          ...
          <p className={cn(
            ...
            avgUtilization > 100 ? "text-rose-600" : "text-slate-900"
          )}>
            {avgUtilization.toFixed(1)}%
          </p>
          <span className="text-[10px] text-slate-400">Ngưỡng quá tải: {simulation?.overloadThreshold ?? 100}%</span>
        </div>
      </div>
