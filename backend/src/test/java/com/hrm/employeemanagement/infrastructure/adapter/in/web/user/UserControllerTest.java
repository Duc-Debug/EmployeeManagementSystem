...
import com.hrm.employeemanagement.infrastructure.adapter.in.web.user.dto.CreateUserRequest;
import com.hrm.employeemanagement.infrastructure.adapter.in.web.user.dto.UpdateUserRoleRequest;
...
class UserControllerTest {
    ...
    void testCreateUser_Returns201CreatedAndLocationHeader() throws Exception {
    ...
    void testCreateUser_DuplicateUsername_Returns409() throws Exception {
        CreateUserRequest request = new CreateUserRequest();
    ...
    void testToggleUserStatus_SelfLocking_Returns400() throws Exception {
        when(toggleUserStatusUseCase.toggleUserStatus(eq(1L), eq(true), nullable(Long.class)))
                .thenThrow(new SelfLockingException("Bạn không thể tự khóa tài khoản của chính mình"));

        mockMvc.perform(patch("/api/v1/users/1/status?lock=true"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value(containsString("tự khóa")));
    }
    ...
    void testGetUserById_NotFound_Returns404() throws Exception {
        when(getUserListUseCase.getUserById(999L))
