...
class PasswordServiceTest {
    ...
    void changePassword_success() {
        ...
        when(loadUserPort.findById(new UserId(10L)))
                .thenReturn(Optional.of(testUser));

        when(passwordEncoder.matches("OldPass123", "old_hash"))
                .thenReturn(true);

        when(passwordEncoder.matches("NewPass123", "old_hash"))
                .thenReturn(false);
    ...
    void changePassword_wrongCurrentPassword_throwsException() {
        ...
        when(passwordEncoder.matches("WrongOldPass", "old_hash"))
                .thenReturn(false);
    ...
    void requestPasswordReset_existingUserWithEmail_invalidatesOldTokensAndSendsEmail() {
        RequestPasswordResetCommand command =
                new RequestPasswordResetCommand("employee1@company.com");

        when(loadUserPort.findByUsernameOrEmail("employee1@company.com"))
                .thenReturn(Optional.of(testUser));

        passwordService.requestPasswordReset(command);

        verify(savePasswordResetTokenPort)
                .invalidateActiveTokensByUserId(testUser.getId());
    ...
    void requestPasswordReset_userWithoutEmail_invalidatesTokensButDoesNotSendEmail() {
        ...
        RequestPasswordResetCommand command =
                new RequestPasswordResetCommand("noemail_user");

        when(loadUserPort.findByUsernameOrEmail("noemail_user"))
                .thenReturn(Optional.of(userWithoutEmail));

        passwordService.requestPasswordReset(command);

        verify(savePasswordResetTokenPort)
                .invalidateActiveTokensByUserId(userWithoutEmail.getId());
    ...
    void resetPassword_validToken_success() {
        ...
        when(loadPasswordResetTokenPort.findByTokenHash(hashedToken))
